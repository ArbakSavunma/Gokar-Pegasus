...
long int rec_ver_transform() {
    ...
    prevyaw=map(rec[0],-20,176,-20,20);
    if(prevyaw<6 && prevyaw>-8) prevyaw=0;
    ...
    prevroll=map(rec[3],-24,174,-20,20);
    if(prevroll<6 && prevroll>-7) prevroll=0; 
    else if(prevroll>20) prevroll=20;
    else if(prevroll<-20) prevroll=-20;
    if(receiver_values[5]>1500){
      throttle=map(rec[2],10,200,0,180);
      if(throttle<20) throttle=0; if(throttle>180) throttle=180;
    }
    else {
      if(prevpitch>3){
        throttle=map(rec[2],10,200,85,180);
        if(throttle<110) throttle=90; if(throttle>180) throttle=180;
        Serial.println(rec[2]);
      }
      else if (prevpitch<-3) {
        throttle=map(rec[2],10,200,0,85);
        if(throttle<20) throttle=0; if(throttle>90) throttle=90;
        Serial.println(rec[2]);
      }
      else {
        ;
...
void mods(long int kademe, struct IMU_Values imu_values) {
  ...
  else if(receiver_values[4]<1700 && receiver_values[4]>1200) {
    Serial.println("*******MOD2********");
  ...
  else if(receiver_values[4]>1700) {
    Serial.println("*******MOD3********");
    ...
    if( receiver_values[5]>1600) {
      ...
      if(-1 * prevyaw<3 && -1 * prevyaw>-3) {
        if(prevpitch>3){
          ...
          if(motpower2<0) motpower2=0;
          if(motpower2>85) motpower2=85;
          ontek1.write(85 - motpower1);
          ontek2.write(85 - motpower2);
